...
## Search
    ...
    public function actionTasktashkilot(){
## Read value
        $draw = isset($_POST['draw']) ? $_POST['draw'] : 1;
        $row = isset($_POST['start']) ? $_POST['start'] : 0;
        $rowperpage = isset($_POST['length']) ? $_POST['length'] : 10; // Rows display per page
...
## Search
        ...
        foreach ($empRecords as $item){
            $data[] = array(
                'id'=>"<button type='button' class='btn btn-success tashikilotadd-{$item->id}' onclick='tashkilotadd({$item->id},\"{$item->name}\")' value='".$item->id."'><span class='fa fa-plus'></span></button>",
                'name'=>"<span class='trtashkilotlist{$item->id}'>{$item->name}</span>",
                'director'=>$item->director,
                'inn'=>$item->inn,
            );
